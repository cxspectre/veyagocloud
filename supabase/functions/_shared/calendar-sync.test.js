/* Tests for _shared/calendar-sync.ts — what sync-outlook-calendar's own
   upsert loop may safely write, and which of what it already has has gone.
   Not tested here (see the file's own header): actually calling Graph or
   Postgres, which needs a live tenant and a live database — the same reason
...
let m;
test.before(async () => {
  const src = fs.readFileSync(path.join(__dirname, 'calendar-sync.ts'), 'utf8');

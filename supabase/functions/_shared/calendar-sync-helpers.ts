/* calendar-sync-helpers.ts — the parts of pulling one connected calendar's
 * events into calendar_events that need no Graph token, no Postgres
 * connection and no clock to get right, so they can be tested here instead
 * of only reasoned about (calendar-sync.test.js, the way graph-message.ts is
 * tested) — deliberately import-free, since the test file loads this module
 * from a data: URL (no relative import can resolve from one), which is also
 * why these live apart from _shared/calendar-sync.ts's own orchestration,
 * even though runCalendarSync() there is their only real caller.
 *
 * Both bugs these exist to fix are things sync-outlook-calendar's own upsert
 ...
 *      deleted in Outlook, and so retired here too. vanishedIds' `truncated`
 *      parameter is the other half of the same fix: a caller whose OWN page
 *      limit ran out partway through must say so, or an event merely past
 *      that point looks exactly as "vanished" as one Outlook genuinely
 *      deleted.
 *
